...
var s_account="yrcdev,yrcwglobaldev"
...
s.linkInternalFilters="javascript:,yrcw.com,myyellow.com,roadway.com,reimerexpress.com,usfc.com,yrc.com,yrcregional.com,regionalexpertise.com"
...
function s_doPlugins(s) {
	...
	s.eVar1=s.getQueryParam('ad_id');
	
	/* Plugin Example: getPagename v2.1 */
	if(!s.pageName) s.pageName=s.getPageName();

	/* Internal campaign report population */
	s.icpath=s.getAndPersistValue(s.eVar1,'s_cp_persist',0);
	if(s.icpath) s.prop4 = s.icpath + " : " + s.pageName;
	if(s.eVar1) s.prop5 = "http://marcomstats.roadway.com/cgi-bin/report/view_ad.pl?ad_id=" + s.eVar1;
	...
	s.eVar36=s.getVisitNum();
		
	/* Plugin Example: getDaysSinceLastVisit v1.1 */
	s.prop8=s.getDaysSinceLastVisit('s_lv');
	if(s.prop8) s.eVar37 = s.prop8;
	
	/* Plugin Example: timeparting EST */
	s.prop9=s.getTimeParting('h','-6','2008'); // Set hour 
	s.prop10=s.getTimeParting('d','-6','2008'); // Set day
	s.eVar38=s.getTimeParting('h','-6','2008'); // Set hour 
	s.eVar39=s.getTimeParting('d','-6','2008'); // Set day

	/* Plugin Example: formAnalysis 2.1  */
	s.setupFormAnalysis(); 
	
	/* Copy props to eVars */
	if(s.pageName) s.eVar2 = s.pageName;
	if(s.prop1) s.eVar3 = s.prop1;
	if(s.prop2) s.eVar4 = s.prop2;
	if(s.prop3) s.eVar5 = s.prop3;
...
s.visitorNamespace="yrcworldwide"
s.trackingServer="yrcworldwide.122.2o7.net"
